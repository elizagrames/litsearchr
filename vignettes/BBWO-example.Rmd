...
BBWO_data <- get_descriptors(BBWO_data, probkeys, stemming=FALSE, exact=FALSE)
...
The trimmed dataset is then passed to make_network() which creates an adjaceny matrix and then uses the package igraph to generate a network from the matrix. When printplot=TRUE, the function saves a high resolution .png file of the original network (without labels) to your current working directory. 
...
BBWO_graph <- make_network(trimmed_dataset, printplot=TRUE)
```
...
```{r}
reduced_graph <- reduce_graph(BBWO_graph, cutoff_strength = cutoffs_cumulative)

search_terms <- get_keywords(reduced_graph, savekeywords = TRUE, makewordle = TRUE)
head(sort(NS, decreasing=TRUE))
