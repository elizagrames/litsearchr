...
```{r}
BBWO_graph <- litsearchr::create_network(BBWO_dfm, min_studies=1, min_occurrences = 1)
```
...
cutoffs_cumulative <- litsearchr::find_cutoff(BBWO_graph, method = "cumulative", cum_pct = .75, diagnostics = TRUE, importance_method = "strength")
```
...
grouped_keywords <- BBWO_grouped_keywords
...
write.csv(similar_terms, "~/Documents/BBWO_similar_terms.csv")

new_terms <- read.csv("~/Documents/BBWO_search_terms_grouped.csv", stringsAsFactors = FALSE)
