---
title: "Introduction to searchr with an example of writing a systematic review search strategy"
author: "Eliza Grames"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to searchr with an example of writing a systematic review search strategy}
...
The **searchr** package for R is designed to partially automate search term selection and writing search strategies for systematic reviews. It uses the Rapid Automatic Keyword Extraction algorithm [@Rose2010] to identify potential keywords from a sample of titles and abstracts and combines them with author- and database-tagged keywords to create a pool of possible keywords relevant to a field of study. Important keywords in a field are identified from their importance in a keyword co-occurrence network. After keywords are grouped into concept groups, **searchr** writes Boolean searches in up to 53 languages, with stemming support for English. The searches work fully in 8 major search databases with partial support in three additional databases.

My intent in creating **searchr** is to make the process of designing a search strategy for systematic reviews easier for researchers in fields that lack Medical Subject Headings (MeSH) by processing the terms commonly used in a field. My hope is that **searchr** can be used to facilitate systematic reviews and contribute to automating evidence synthesis. 

## Acknowledgements

I want to thank:
 - Chris Elphick for feedback, support, and advice at every stage of this project
 - Carolyn Mills for advice on which databases to search, testing database search rules, and guiding me through the process of manually writing a search strategy
 - Andrew Stillman and Morgan Tingley for letting me use their study system as an example and for feedback and ideas at all stages of this project
 ...
 - Developers of the packages on which **searchr** relies: dplyr, freeknotsplines, gdata, igraph, knitr, quanteda, rapidraker, rmarkdown, SnowballC, splines2, stringr, tm, and wordcloud.


## Comments, suggestions, bugs, and questions

**searchr** is a work in progress - any and all comments, suggestions, bugs, or questions are welcome! Please email me at eliza.grames\@uconn.edu or open an issue at  https://github.com/elizagrames/searchr.

# Installation and dependencies

To install **searchr** on a Unix-based OS (OSX or Linux) use devtools::install_github("elizagrames/searchr")
To install **searchr** on a Windows machine, download the repository from https://github.com/elizagrames/searchr, delete the ./src/ directory, and build the package. Or, if you're on Windows and know better than me about how to install R packages from GitHub that were built on a Unix system, let me know so I can update the instructions!
