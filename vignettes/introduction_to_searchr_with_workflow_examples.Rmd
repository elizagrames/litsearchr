...
```{r}
raked_keywords <- extract_terms(BBWO_data, new_stopwords = NULL, min_freq = 2, title = TRUE, abstract = TRUE, ngrams = TRUE, n=2)
...
```{r}
real_keywords <- select_actual_terms(BBWO_data, ngrams = TRUE, n=2)
...
```{r}
cutoffs_spline <- find_cutoff(BBWO_graph, method = "spline", degrees = 2, knot_num = 4, diagnostics = TRUE, importance_method = "strength")
...
write_stemmed_search(groupdata = my_search_terms, exactphrase = TRUE, directory="./")

```


Now we can inspect our written searches. Foreign language searches will be in unicode, but if you open them in a web browser, you can copy and paste them into search databases (assuming you have the appropriate fonts on your computer to display them). 

```{r}
readLines("./search-in-Russian.txt")
