...
```{r}
BBWO_import <- litsearchr::import_naive("../inst/extdata/", remove_duplicates = FALSE, clean_dataset = TRUE)
BBWO_data <- litsearchr::deduplicate(BBWO_import, use_abstracts = FALSE, use_titles=TRUE, title_method = "tokens", title_sim = .8)
BBWO_data <- litsearchr::deduplicate(BBWO_import, use_abstracts = TRUE, use_titles=FALSE, doc_sim = .8)
...
Fully supported databases (tested on these platforms):
...
Partial support:
 ...
 - ScienceDirect [no wildcards, no more than 8 boolean operators; not recommended]
