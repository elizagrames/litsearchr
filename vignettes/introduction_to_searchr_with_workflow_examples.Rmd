...
To install **litsearchr** on a Unix-based OS (OSX or Linux) use devtools::install_github("elizagrames/litsearchr")
To install **litsearchr** on a Windows machine, download the repository from https://github.com/elizagrames/litsearchr, delete the ./src/ directory, and build the package. Or, if you're on Windows and know better than us about how to install R packages from GitHub that were built on a Unix system, let us know so we can update the instructions!
...
We start with a naive search, which is a search of a few databases (namely, Scopus, BIOSIS, and Zoological Record) using a simple Boolean search, then use litsearchr to identify additional relevant keywords that are used in the literature but aren't in our naive search, and finally we write new Boolean searches using the newly identified search terms to conduct a systematic review in multiple languages. 
...
At this stage, you can of course also add in additional terms that you've thought of that weren't in your naive search and weren't found by litsearchr. litsearchr is merely meant as a tool to identify additional keywords - not to restrict your search. 
...
# For the bird group, we removed our naive term "bird" because it isn't specific to woodpeckers
bird_group <- append(
...
The functions to write searches write simple Boolean searches that work in 14 tested databases.
